...
## Ajout du 13/09/2026 (soir) — page CTMS repositionnée

- **Positionnement revu** (consigne de Lydie) : plus de « petites structures » ni de « remplacer les trackers Excel » (trop réducteur). Hero : « le CTMS modulaire qui réunit tout le pilotage de vos études » ; différenciateurs = tout au même endroit (CTMS classique + suivis annexes : trackers ARCs, weekly calls, formations RH), interconnexion de bout en bout (une saisie se propage partout), simplicité de prise en main, moins de charge mentale (traçabilité en arrière-plan, focus projet/patients/safety). Section « Ce qu'OKILYS CTMS a que les autres n'ont pas ».
...
## Session laptop 14 → 16/09/2026 — récapitulatif

- **Reprise sur le laptop** (utilisateur Windows `LydiePARSUS{Okilys}`) : dépôt vérifié, `git pull` / `git push` fonctionnels en ligne de commande, mémoires Claude reconstituées puis fusionnées avec l'export du PC fixe du 14/09.
- **Consignes de Lydie fixées pendant la session** : la session Site Web ne traite que le site (le technique et la mise en ligne d'OKILYS Suite restent dans la session OKILYS Suite ; une demande envoyée dans la mauvaise fenêtre est transmise mot pour mot à la bonne session) ; chaque modification doit aussi améliorer le référencement de ce qui est modifié (titres, descriptions, fiches Google, adresses, menus, FR + EN) ; les spécifications de tests du site (`_notes/TEST-SPECIFICATION-site-web.md`) se mettent à jour ici puis se transmettent à la session de tests avec sa consigne standard ; fichiers de résultats à un seul onglet au format « registre des défauts ».
- **Page CTMS** : accroche élargie (trackers, calls, formations, gestion et allocation de l'équipe, répertoire des accès aux systèmes) ; sections réordonnées (différenciateurs → socle/modules/outils → vue d'ensemble → Espace de travail → rôles → sécurité → comment ça se passe) ; nouvelle section **Espace de travail** (page d'accueil de l'outil : tâches et rappels toutes études, photo `ctms-workspace.jpg`) ; registre des accès aux systèmes dans les outils ; modules thématiques (oversight promoteur ICH E6(R3)) dans la carte des modules et la fiche Google ; répétitions retirées.
- **Référencement** : audit des 40 pages sans défaut structurel ; titres/descriptions raccourcis (CTMS, suite, eTMF, actualités, pratique courante) ; tirets longs retirés des titres en police Agne (cartes CTMS, mentions légales) ; pied de page aligné sur le menu (Secteurs, Contact) ; repères de sections renommés (voir ajout du 15/09) ; version `20260915a`.
- **Tests** : spécification v1.1 (~130 cas), harnais `_notes/tests/site_audit.py` et générateurs de classeurs, baseline du 15/09 : 119 cas, 84 réussite / 6 mineurs / 38 bloqués ; points remontés : pas de DMARC sur okilys.com, pas d'en-têtes de sécurité (GitHub Pages), clé Web3Forms à restreindre au domaine, fichier `assets/images/presentation-equipe.jpg` (2,7 Mo) non référencé.
- **Reste ouvert** : Magdalena, champ d'action OKILYS sur les 5 pages produits, décisions DMARC / CSP / Web3Forms, module Notes de frais (le mentionner ou non), carte dédiée à l'oversight dans les différenciateurs, titre de `suite.html` (72 caractères), question de la mémoire unique à reposer le 17/09.
- **Transfert vers le PC fixe** : `git pull` pour le site ; mémoires et `.claude/launch.json` préparés dans `OKILYS\_transfert-laptop\retour-pc-fixe\` (procédure dans `RETOUR-PC-FIXE.md`).
