...
Version 2.2 - 20 September 2026 (night) - **`OKILYS Freelance 2` confirms the same picture for `freelance.okilys.com`; nothing to change on the showcase site.** Checked by that session in the code and on the server on 20/09:
- **No e-mail is sent** by either installation. `server/mailer.py` is dormant and would only activate with `CTMS_SMTP_HOST`, which is set on neither instance; the application returns the text on screen and Lydie PARSUS sends it from her own mailbox. **The 18/10 move to `p=quarantine` is confirmed risk-free for both sub-domains.** Standing caveat recorded by both sessions: the module's default sender is `ctms@okilys.com` and would leave from the VPS, failing SPF `-all`, so authenticated Microsoft 365 is the only acceptable route if sending is ever switched on - and they will consult this session first.
- **Certificate confirmed Let's Encrypt** (issuer read on the server, `CN=freelance.okilys.com`, to 16/12/2026), HTTP-01, no wildcard. The apex CAA is compatible.
- **`freelance` is an internal tool, not a product**: decision of Lydie PARSUS of 17/09/2026, single-user, used for one client assignment where the CTMS is not wanted. **It therefore has no place in the site's suite bar, menu, home cards or sitemap, and its absence is deliberate - not an omission.** A future session must not "fix" this by adding it. Any change here is an editorial decision belonging to Lydie alone.
- **Scope boundary worth stating once**: the application processes personal data (investigator-site staff, sponsor contacts, user accounts; no patient data, subjects appear only as counters) and **serves its own `/legal` and `/privacy` pages**. Our `mentions-legales.html` / `en/legal-notice.html` cover **the showcase site only** - essentially the contact form relayed by Web3Forms. **The application's processing is out of this specification's scope and must not be added to the site's legal pages**; that common content is piloted on the CTMS side. No case is added here.
- **No third-party call from the browser** on freelance either (strict CSP, self-hosted fonts): nothing equivalent to our Web3Forms exposure. The showcase site remains the only surface with an external call.

(Version 2.1 - 20 September 2026 (night) - **Confirmations from the current CTMS session (`OKILYS CTMS 2`), checked by it on 20/09, not quoted from a note.** The earlier answers came from a frozen session (`OKILYS Suite`) and were re-asked after Lydie PARSUS pointed out the mistake.
- **The CTMS application sends no e-mail at all**, verified three ways: `server/mailer.py` is imported nowhere, no `CTMS_SMTP_*` variable is set on either instance, and no MTA is installed with nothing listening on port 25. **The move to `p=quarantine` around 18/10 therefore carries no risk for it.** Recorded for later: if sending is ever switched on, it must go through authenticated Microsoft 365, never straight from the VPS, or SPF `-all` will have the messages rejected. The CTMS session will warn before that happens.
- **Certificates confirmed Let's Encrypt** for both `ctms` (to 09/12/2026) and `freelance` (to 16/12/2026), issued by `certbot --nginx`, HTTP-01, no wildcard. **New standing caveat for PT-DNS-04**: our apex CAA authorises Let's Encrypt *only*, so **any future sub-domain whose certificate comes from another authority (OVH, Gandi, Cloudflare...) needs its own CAA entry added first**, or issuance is refused. This is a trap that would surface weeks later, at renewal time.
- **`etmf.okilys.com` is planned** - the CTMS code already carries a `CTMS_ETMF_URL` variable, empty today, and the suite bar shows the "à venir" badge. Nothing is published in DNS yet, so PT-DNS-03 rightly expects it absent. **When it appears, PT-DNS-03's sub-domain list and this expectation must both be updated**, and the CAA caveat above applies to it. No other sub-domain is planned.
- Out of scope for this site but recorded so the picture is complete: the CTMS repository is **local git only**, so Lydie's GitHub points (2FA, branch protection) do not apply to it; its equivalent risk is that the code has no copy off the PC. Backups became encrypted on 20/09 and off-server copies are being delivered.
- **Gandi's registrar lock and 2FA protect the CTMS as well**, since `ctms.okilys.com` lives in this zone: whoever controls the DNS can divert the address of the tool Lydie works with daily. That raises the practical priority of that item well above "zone hygiene".)

(Version 2.0 - 20 September 2026 (night) - **The editorial contradiction is resolved: OKILYS CTMS is announced as forthcoming everywhere, news included.** Decision of Lydie PARSUS on 20/09 ("corrige pour que ca match avec a venir"), applied in both languages in the same intervention:
- `actualites.html` / `en/insights.html`: the article **"OKILYS CTMS est lance" / "is live" becomes "OKILYS CTMS arrive" / "is coming"**, its eyebrow becomes **"À venir" / "Coming soon"** like the eTMF article, and the body moves to the future tense. **The anchors `#lancement-ctms` and `#ctms-launch` are deliberately kept**: they are already shared on LinkedIn and indexed, and renaming them would break those links for no editorial gain.
- Home carousel, FR + EN: same title, same eyebrow, body reworded. Both upcoming tools now read "À venir" / "Coming soon" side by side.
- **Search-engine metadata realigned** on all four description slots per page (`description`, `og:description`, `twitter:description`, JSON-LD): "nouveau site, lancement d'OKILYS CTMS, preparation de l'eTMF" becomes "OKILYS CTMS et eTMF a venir, nouveau site bilingue" (129 characters FR, 125 EN, both inside the 70-160 target). `sitemap.xml` `lastmod` moved to 2026-09-20 for the four pages touched.
- **New case UT-PAGE-22**, a regression guard over all 51 pages: no page may claim OKILYS CTMS is already launched. Forward-looking wording ("Etre prevenu du lancement") is deliberately not matched. This guard exists because the CTMS session twice overwrote pages of this site through a generator, so the contradiction could silently come back.

No CSS and no JavaScript changed, so the `?v=` version tag is untouched. Verified in the browser on the local server, FR and EN: both articles render, both carousels show two "À venir" / "Coming soon" tools, empty console.)

(Version 1.9 - 20 September 2026 (evening) - **PT-DNS-04 closed in Pass: the CAA record is published.** Lydie PARSUS created it at Gandi on 20/09: `okilys.com. CAA 0 issue "letsencrypt.org"`, confirmed on the wire against the authoritative server `ns-72-a.gandi.net` (flags 0, tag `issue`, value `letsencrypt.org` - well formed; the quotes are presentation only and do not exist on the wire, so Gandi's unquoted preview was correct). Two lessons recorded in the harness:
- **A single DoH resolver is not enough.** Minutes after the record was created, `dns.google` still served its cached negative answer while `cloudflare-dns.com` already saw the record - a false Fail. `doh()` now queries both and the **first non-empty answer wins**; Blocked is reported only when every resolver fails.
- **CAA follows CNAMEs (RFC 8659).** `www.okilys.com` is a CNAME to `okilyslifescience.github.io`, so certificate issuance for `www` is governed by **GitHub's** CAA, not by ours - GitHub publishes `issue`/`issuewild` for `letsencrypt.org`, `sectigo.com` and `digicert.com`. Our apex record governs `okilys.com` itself and the A-record sub-domains `ctms` and `freelance`, which inherit it. All three are compatible with Let's Encrypt, so nothing is at risk. A future session must not read the GitHub CAA as an anomaly.

**Remaining PT failure: PT-DNS-06 (DNSSEC) only.**) Recommended check before December: `certbot renew --dry-run` on the VPS (CTMS session, on Lydie's decision) and a certificate renewal observed for the site.

(Version 1.8 - 20 September 2026 (evening) - **DNS coverage corrected and widened after the CTMS session's reply.** Four changes, all in the PT-DNS family: (a) **PT-DNS-03 was mislabelled**: the harness ran the SPF check under the "sub-domain takeover" id, so takeover was **never actually tested**. PT-DNS-03 now performs the real check over an explicit sub-domain list and SPF moves to the new **PT-DNS-07**. (b) **A third sub-domain was discovered on 20/09**: `freelance.okilys.com`, on the same VPS as ctms (141.94.42.79) - it appeared in no site-session document and is now part of the tested list (`www`, `ctms`, `freelance`, `etmf`). (c) **PT-DNS-04 (CAA), PT-DNS-05 (apex/www) and PT-DNS-06 (DNSSEC) are added to this table**: the harness computed them but the report is built from *this* table, so they were silently dropped. (d) **The harness could never see a CAA or DS record**: Windows `nslookup` answers "unknown query type" for both, so those checks are now made over DNS-over-HTTPS (dns.google), and a lookup failure yields Blocked rather than a false Fail. **Two genuine failures surface as a result, both outside the repository and both actions for Lydie PARSUS**: no CAA record (PT-DNS-04) and **DNSSEC is not active** (PT-DNS-06) - no DS at the .com registry and no DNSKEY in the zone, which **contradicts the 19/09 note claiming DNSSEC was active**; that note was wrong and is corrected here. **The CAA precondition of v1.7 is now cleared**: the CTMS session confirmed on 20/09 that both `ctms.okilys.com` and `freelance.okilys.com` use Let's Encrypt, renewed by `certbot.timer`, with no wildcard - so `0 issue "letsencrypt.org"` at the apex is safe for the whole zone. Also recorded from that reply: **the CTMS application sends no e-mail at all** (decision of Lydie, 11/09), so the move to `p=quarantine` on 18/10 carries no risk for it.)

(Version 1.7 - 20 September 2026 - **PT-DNS-01 closed in Pass: mail authentication is now complete for `okilys.com`.** Done with Lydie PARSUS on 20/09 and verified from an external resolver (8.8.8.8): (a) **DKIM was never enabled** - discovered during the session; it was activated in Microsoft 365 with the two CNAME records published at Gandi (`selector1._domainkey` / `selector2._domainkey` -> `selectorN-okilys-com._domainkey.clinitriasccom.p-v1.dkim.mail.microsoft.`), Microsoft flipped the toggle by itself on detection, status `Valid`, RSA public key resolving; (b) **DMARC published** at `_dmarc.okilys.com` as `v=DMARC1; p=none; rua=mailto:contact@okilys.com; adkim=s; aspf=s; pct=100`. **DKIM was deliberately enabled before DMARC**: with SPF alone the alignment breaks as soon as a message is forwarded, so `adkim=s` would have rested on nothing. The policy is intentionally `p=none` (observation) for four weeks; **PT-DNS-01 stays partially open until the switch to `p=quarantine` planned around 18/10/2026**, which is a registrar action for Lydie. PT-DNS-01's expected result is updated to require DKIM explicitly. The `clinitriasccom.onmicrosoft.com` tenant domain stays `NoDKIMKeys` - out of scope, no mail leaves under that name. **PT-DNS-02 (CAA) gains a precondition**: before authorising `letsencrypt.org` only, confirm which authority issues the certificate of `ctms.okilys.com` (VPS, other session) - a CAA that excludes it would break that renewal. **No defect is open in the repository; the automated harness now reports 0 Fail.**)
...
| Hosting and domain | CNAME, HTTP → HTTPS, apex → www, TLS certificate, DNS (SPF, DKIM, DMARC, CAA, DNSSEC, sub-domains `ctms` / `freelance` / `etmf`) | - | IT-HOST | - | PT-TLS, PT-DNS, PT-INFO |
...
| UT-PAGE-22 | Coming-soon consistency | All 51 pages scanned for claims that OKILYS CTMS is already launched (`est lancé`, `is live`, `lancement d'OKILYS CTMS`, `launch of OKILYS CTMS`, `est disponible`, `is now available`). Forward-looking wording such as `Être prévenu du lancement` is intentionally not matched | no page claims the tool is launched, FR and EN | Le site ne se contredit pas : OKILYS CTMS est annoncé « à venir » partout |
...
| PT-DNS-01 | Mail authentication | SPF, DMARC at `_dmarc.okilys.com`, and the two DKIM selectors `selector1._domainkey` / `selector2._domainkey` resolved from an external resolver; the DKIM CNAME must resolve through to a `v=DKIM1` public key, not merely exist | SPF `-all`; **DMARC published** with `rua=`; **DKIM enabled and its public key resolving**. `p=none` accepted until 18/10/2026 (declared observation window), `p=quarantine` required afterwards | Les e-mails « @okilys.com » ne peuvent pas être usurpés — signature DKIM active et DMARC publié depuis le 20/09 |
| PT-DNS-02 | Zone hygiene | CAA record (`letsencrypt.org` for GitHub Pages — **precondition: confirm the certificate authority used by the `ctms.okilys.com` VPS before restricting the zone, or its renewal breaks**), DNSSEC status, no wildcard, no dangling CNAME on `etmf`, `ctms`, `www`; registrar lock and 2FA at the registrar | no takeover possible; CAA present or decision recorded | Aucun sous-domaine abandonné, nom de domaine verrouillé |
| PT-DNS-03 | Sub-domain takeover | Explicit list resolved over DoH: `www` (GitHub Pages), `ctms` and `freelance` (OVH VPS 141.94.42.79), `etmf` (must stay absent). A record set containing a CNAME but no final address = dangling target = takeover signature. **The list is part of the case: any new sub-domain must be added here.** Until 20/09/2026 this id wrongly carried the SPF check | no dangling sub-domain; an absent sub-domain is a Pass (nothing to hijack) | Aucun sous-domaine récupérable par un tiers |
| PT-DNS-04 | CAA | CAA record at the apex of okilys.com, read over DoH (Windows `nslookup` cannot query this type) | a CAA authorising the certificate authority actually used. **Verified on 20/09: `ctms` and `freelance` both use Let's Encrypt**, so `0 issue "letsencrypt.org"` covers the whole zone | Seule une autorité déclarée peut délivrer un certificat au nom d'okilys.com |
| PT-DNS-05 | Apex and www resolution | apex and `www` resolve to the GitHub Pages addresses | both resolve, site reachable | Le site répond bien sur okilys.com et www.okilys.com |
| PT-DNS-06 | DNSSEC | DS record at the `.com` registry and DNSKEY in the zone, read over DoH | DNSSEC active, **or** the decision not to activate it recorded. *Measured 20/09/2026: neither DS nor DNSKEY - DNSSEC is NOT active, contrary to the 19/09 note* | Les réponses DNS du domaine sont signées et non falsifiables |
| PT-DNS-07 | SPF policy | SPF record of okilys.com (`v=spf1 include:spf.protection.outlook.com -all` observed) | SPF present, ending in `-all`. Was carried by PT-DNS-03 until 20/09/2026 | Seuls les serveurs déclarés peuvent envoyer au nom d'okilys.com |
...
- ~~DMARC (and DKIM) for `okilys.com`~~ — **done on 20/09/2026** (PT-DNS-01 Pass). Residual follow-up, outside the repository: move the policy from `p=none` to `p=quarantine` around 18/10/2026, after the observation window.
...
- ~~CAA DNS record absent~~ - **published on 20/09/2026** (PT-DNS-04 Pass). Residual follow-up: confirm a real renewal before 09/12 and 16/12 (`certbot renew --dry-run` on the VPS, CTMS session, on Lydie's decision).
- **DNSSEC not active** (PT-DNS-06, Fail since 20/09/2026): no DS at the `.com` registry, no DNSKEY in the zone - the 19/09 note claiming DNSSEC was active was wrong. Low severity (needs an attacker already on the network path). One-click at Gandi, but a botched key rollover makes the domain unreachable: to be decided with Lydie PARSUS, not done silently.
