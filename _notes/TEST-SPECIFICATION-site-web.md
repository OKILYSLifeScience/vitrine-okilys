...
- Suite bar (above the header on every page): OKILYS website (current) · OKILYS CTMS → https://ctms.okilys.com · OKILYS eTMF badged *à venir / coming soon*.
- Contact form: fields name, email, organisation (optional), message; hidden `access_key`, `subject`, `from_name`; honeypot checkbox `botcheck`; messages FR / EN for sending, sent, failed, network error.
...
| Home (FR / EN) | Hero, figures band, news carousel, À propos + HOME values tiles, Produits de santé (5 cards), Expertises (7 steps), Secteurs, OKILYS Suite panel, Contact (form + e-mail / phone) | UT-JS-01, 07..14 | IT-FORM | ST-HOME, ST-CONTACT | PT-FORM, PT-XSS |
...
| IT-NAV-05 | home carousel buttons "Lire l'actualité" | each → `actualites.html#<anchor>` existing (EN: `insights.html#<anchor>`) | Accueil › Actualités : le bouton ouvre le bon article |
| IT-NAV-06 | OKILYS Suite panel on the home page | 2 cards → `ctms.html`, `etmf.html`; button "Comment ces outils sont nés" → `suite.html` | Accueil › encart OKILYS Suite : les cartes et le bouton mènent aux bonnes pages |
| IT-NAV-07 | CTA buttons | "Parlons de votre étude" (ctms, etmf, suite) → `index.html#contact`; "Accès client" → `https://ctms.okilys.com` | Boutons d'appel à l'action : vers le contact ou vers l'accès client |
...
| PT-LINK-01 | External links | every `target="_blank"` has `rel="noopener"` (171 internal absolute links excluded); every external domain (LinkedIn, ctms.okilys.com, infrarch.fr, client sites, cnil.fr, github.com) resolves to the intended organisation over HTTPS; no typosquatted or expired domain | no reverse-tabnabbing; no link to an expired domain that could be re-registered | Aucun lien sortant piégeable ou vers un domaine abandonné |
