...
- **CAA DNS record absent** (PT-DNS-04, Fail since 20/09/2026): add `0 issue "letsencrypt.org"` at the apex. **Precondition cleared on 20/09** - both VPS sub-domains use Let's Encrypt, no wildcard. Advice from the CTMS session: publish it well before early December (certificates renew 09/12 and 16/12) and run `certbot renew --dry-run` afterwards. Registrar access required.
